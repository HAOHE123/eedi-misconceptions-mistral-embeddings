...
sentence_embeddings = recall_inference(misconception_mapping, model, tokenizer, device, batch_size, max_length)

index_text_embeddings_index = {index: paper_id for index, paper_id in enumerate(list(sentence_embeddings.keys()))}
...
# query embedding
query_embedding_train = recall_inference(train, model, tokenizer, device, batch_size, max_length)
query_embedding_valid = recall_inference(valid, model, tokenizer, device, batch_size, max_length)
# del model
gc.collect()
torch.cuda.empty_cache()

RANK = 100
predict_train = get_predict(train, query_embedding_train, sentence_embeddings, index_text_embeddings_index, device, RANK=RANK)
...
RANK = 100
predict_valid = get_predict(valid, query_embedding_valid, sentence_embeddings, index_text_embeddings_index, device, RANK=RANK)
valid['top_recall_pids'] = predict_valid


if lora_path != 'none':
    valid.to_csv(f'{output_path}valid.csv', index=False)
